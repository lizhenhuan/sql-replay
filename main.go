...
func main() {
	var mode string
	flag.StringVar(&mode, "mode", "", "Mode of operation: parsemysqlslow, parsetidbslow, parsemysqlcsv, parsesqlserver, parsetencentaudit, replay, load, report")
	...
	default:
		fmt.Println("Invalid mode. Available modes: parsemysqlslow, parsemysqlcsv, parsetidbslow, parsesqlserver, parsetencentaudit, replay, load, report")
		os.Exit(1)
	}
}

func printUsage() {
	fmt.Println("Usage: ./sql-replay -mode [parsemysqlslow|parsemysqlcsv|parsetidbslow|parsesqlserver|parsetencentaudit|replay|load|report]")
	...
	fmt.Println("    5. replay mode: ./sql-replay -mode replay -db <mysql_connection_string> -speed 1.0 -slow-out <slow_output_file> -replay-out <replay_output_file> -username <all|username> -sqltype <all|select> -dbname <all|dbname> -ignoredigests <digest1,digest2...> -lang <en|zh>")
	fmt.Println("    6. load mode: ./sql-replay -mode load -db <DB_CONN_STRING> -out-dir <DIRECTORY> -replay-name <REPORT_OUT_FILE_NAME> -table <replay_info>")
	fmt.Println("    7. report mode: ./sql-replay -mode report -db <mysql_connection_string> -replay-name <replay name> -port ':8081'")
}
