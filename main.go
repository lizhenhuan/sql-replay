...
func main() {
	var mode string
	flag.StringVar(&mode, "mode", "", "Mode of operation: parsemysqlslow, parsetidbslow, parsemysqlcsv, parsesqlserver, parsetencentaudit, parsealiaudit, parseoracle, replay, load, report, cloudreport")
	...
	case "parsealiaudit":
		ParseAliAuditCSV(slowLogPath, slowOutputPath)
	case "replay":
	...
	default:
		fmt.Println("Invalid mode. Available modes: parsemysqlslow, parsemysqlcsv, parsetidbslow, parsesqlserver, parsetencentaudit, parsealiaudit, parseoracle, replay, load, report, cloudreport")
		os.Exit(1)
	}
}

func printUsage() {
	fmt.Println("Usage: ./sql-replay -mode [parsemysqlslow|parsemysqlcsv|parsetidbslow|parsesqlserver|parsetencentaudit|parsealiaudit|parseoracle|replay|load|report]")
	...
	fmt.Println("    5. parse Alibaba Cloud RDS audit csv: ./sql-replay -mode parsealiaudit -slow-in <path_to_audit_csv> -slow-out <path_to_slow_output_file>")
	fmt.Println("    6. parse Oracle AWR report: ./sql-replay -mode parseoracle -slow-in <path_to_awr_html_or_dir> -slow-out <path_to_slow_output_file>")
	fmt.Println("    7. replay mode: ./sql-replay -mode replay -db <mysql_connection_string> -speed 1.0 -slow-out <slow_output_file> -replay-out <replay_output_file> -username <all|username> -sqltype <all|select> -dbname <all|dbname> -ignoredigests <digest1,digest2...> -lang <en|zh>")
	fmt.Println("    8. load mode: ./sql-replay -mode load -db <DB_CONN_STRING> -out-dir <DIRECTORY> -replay-name <REPORT_OUT_FILE_NAME> -table <replay_info>")
	fmt.Println("    9. report mode: ./sql-replay -mode report -db <mysql_connection_string> -replay-name <replay name> -port ':8081'")
	fmt.Println("    10. cloudreport mode: ./sql-replay -mode cloudreport -db <mysql_connection_string> -replay-name <replay name> -port ':8081'")
}
